...
# generate_heart_command analysisService config threshold except-listeners only-listeners
...
command=$(generate_heart_command "$analysisService" "$config" "$threshold" "$exceptServices" "$onlyServices")
npx heart $command
